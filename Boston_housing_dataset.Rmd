...
boston_final_model <- gam(MEDV ~ CHAS + RAD + s(ZN) + s(PTRATIO) + s(TAX) + s(INDUS) + s(NOX,sp=0.01) + s(B,sp=8) + s(AGE) + s(DIS,sp=0.01) + s(RM,sp=1) + s(LSTAT) + s(CRIM), data = boston_baked_train) 
...
What happens if we only use the variables with splines to predict the response `MEDV`? 
As we've seen from the attempt in `ames`, the RMSE of only using splines as the explanatory variables had reduced the RMSE. 
Would that work the same here?
