...
From the formula above, we know that, the more wiggly we want the variable to fit, the smaller `λ` needs to be (between 0 and 1), and if we want the fit to be less wiggly, `λ` has to be bigger than 1. 

```{r}
plot(ames_final_model2, residuals=TRUE,shade = TRUE, shade.col = "lightblue")
```
